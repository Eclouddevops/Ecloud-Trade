...
class BreakoutProbability:
    ...
    def calculate(self, df: pd.DataFrame) -> dict:
        ...
        return {
            "breakout_probability": float(round(base_probability, 1)),
            "upside_probability": int(upside_prob),
            "downside_probability": int(downside_prob),
            "signal": signal,
            "confidence": confidence,
            "direction_bias": float(round(direction_bias * 100, 1)),
            "factors": {
                ...
                "bb_squeeze": float(round(squeeze_ratio, 3)),
